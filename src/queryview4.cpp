	...
	m_pLastItem(NULL)
{
	...
	connect(this, SIGNAL(doubleClicked(QTableViewItem*)), this, 
		SLOT(slotRecordSelected(QTableViewItem*)));
	connect(this, SIGNAL(returnPressed(QTableViewItem*)), this, 
		SLOT(slotRecordSelected(QTableViewItem*)));
		
	// Show the popup-menu when requested
	connect(this, 
		SIGNAL(contextMenuRequested(QTableViewItem*, const QPoint&, int)),
		m_pQueryMenu, SLOT(slotShow(QTableViewItem*, const QPoint&, int)));
		
	// Handle popup-menu commands
	connect(m_pQueryMenu, SIGNAL(viewSource(QTableViewItem*)), this,
		SLOT(slotRecordSelected(QTableViewItem*)));
	connect(m_pQueryMenu, SIGNAL(findDef(const QString&)), this,
		SLOT(slotFindDef(const QString&)));
	connect(m_pQueryMenu, SIGNAL(copy(QTableViewItem*, int)), this,
		SLOT(slotCopy(QTableViewItem*, int)));
	connect(m_pQueryMenu, SIGNAL(filter(int)), this, SLOT(slotFilter(int)));
	connect(m_pQueryMenu, SIGNAL(showAll()), this, 
		SLOT(slotShowAll()));
	connect(m_pQueryMenu, SIGNAL(remove(QTableViewItem*)), this,
		SLOT(slotRemoveItem(QTableViewItem*)));
}
...
void QueryView::addRecord(const QString& sFunc, const QString& sFile,
	const QString& sLine, const QString& sText, QTableViewItem* /* pParent */)
{
	QTableViewItem* pItem;
 ...
 */
void QueryView::select(QTableViewItem* pItem)
{
 ...
 */
void QueryView::queryFinished(uint nRecords, QTableViewItem*)
{
 ...
 * NOTE: We override this method since the QTableView implementation opens
 ...
 */
void QueryView::slotRecordSelected(QTableViewItem* pItem)
{
 ...
 */
void QueryView::slotCopy(QTableViewItem* pItem, int nCol)
{
 ...
 */
void QueryView::slotRemoveItem(QTableViewItem* pItem)
{
	delete pItem;
}
